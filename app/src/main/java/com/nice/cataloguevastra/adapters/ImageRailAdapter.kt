...
class ImageRailAdapter(
    ...
    class VisualViewHolder(
        ...
        fun bind(item: RailItemUiModel.Visual) = with(binding) {
            when {
                item.imageUri != null -> previewImage.setImageURI(item.imageUri)
                !item.imageUrl.isNullOrBlank() -> previewImage.load(item.imageUrl) {
                    placeholder(item.imageRes)
                    error(item.imageRes)
                    crossfade(true)
                }
                else -> previewImage.setImageResource(item.imageRes)
            }
